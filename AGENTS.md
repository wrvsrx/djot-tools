...
This is a **Cargo workspace** (`crates/*`) so the djot semantics can be shared by more than one tool: an exporter (djot → pandoc AST) is planned alongside the language server.
