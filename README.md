...
- a definition list marked with `{.metadata}` is lifted out of the rendered
  body and folded into pandoc metadata;
- definition values are Djot content, so a `title` (or any field) may contain
  emphasis, math, or a `[@key]{.cite}` citation; a verbatim-only value becomes
  `MetaString`, bullet lists recursively become `MetaList`, and other rich or
  multi-block content keeps its Pandoc structure;
...
- [`crates/djot-core`](crates/djot-core) – protocol-agnostic djot analysis
  (heading outline, anchor/reference index, metadata-field extraction) working
